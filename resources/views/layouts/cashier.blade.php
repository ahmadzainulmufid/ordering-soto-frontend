...
<head>
    ...
    <title>@yield('title', 'Cashier Panel - Soto Lamongan Cak Mufid')</title>
    ...
    <aside class="left-0 top-0 h-screen w-72 fixed bg-secondary text-on-secondary shadow-md flex flex-col py-6 z-50">
        ...
        <div class="px-6 mb-8 flex items-center gap-3">
            ...
            <div class="min-w-0">
                <h1 class="font-title-md text-title-md text-primary-container font-bold truncate">
                    {{ session('user.full_name', 'Cashier') }}
                </h1>
        ...
        <nav class="grow space-y-1 overflow-y-auto custom-scrollbar px-2">
            <a href="{{ route('cashier.orders.index') }}"
                class="flex items-center gap-3 rounded-lg px-4 py-3 mx-2 transition-all group {{ request()->routeIs('cashier.orders.*') ? 'bg-primary-container text-on-primary-container font-bold shadow-md translate-x-1' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                <span class="material-symbols-outlined">receipt_long</span>
                <span class="font-label-md text-label-md">Kelola Pesanan</span>
            </a>

            <a href="{{ route('cashier.menu.stock') }}"
                class="flex items-center gap-3 rounded-lg px-4 py-3 mx-2 transition-all group {{ request()->routeIs('cashier.menu.stock') ? 'bg-primary-container text-on-primary-container font-bold shadow-md translate-x-1' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                <span class="material-symbols-outlined">inventory_2</span>
                <span class="font-label-md text-label-md">Stok Menu</span>
            </a>
        </nav>
